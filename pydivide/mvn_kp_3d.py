...
def mvn_kp_3d(kp):
    ...
    def set_time(evt):
        ...
        sun.direction = (math.cos(solar_lon[value] * math.pi / 180), math.sin(solar_lon[value] * math.pi / 180),
                         math.sin(solar_lat[value] * math.pi / 180))
        sun_axis.axis = vector(3 * math.cos(solar_lon[value] * math.pi / 180),
                               3 * math.sin(solar_lon[value] * math.pi / 180),
                               3 * math.sin(solar_lat[value] * math.pi / 180))
...
###############################################################################
    ...
    geo_x_axis = arrow(pos=vector(0, 0, 0), axis=vector(3, 0, 0), color=color.white, shaftwidth=0.03, headwidth=.06,
                       opacity=0)
    geo_y_axis = arrow(pos=vector(0, 0, 0), axis=vector(0, 3, 0), color=color.white, shaftwidth=0.03, headwidth=.06,
                       opacity=0)
    geo_z_axis = arrow(pos=vector(0, 0, 0), axis=vector(0, 0, 3), color=color.white, shaftwidth=0.03, headwidth=.06,
                       opacity=0)
    sun = distant_light(direction=(math.cos(solar_lon[0] * math.pi / 180), math.sin(solar_lon[0] * math.pi / 180),
                                   math.sin(solar_lat[0] * math.pi / 180)), color=color.white)
    ...
    sun_axis = arrow(pos=vector(0, 0, 0), axis=vector(3 * math.cos(solar_lon[0] * math.pi / 180), 3 *
                                                      math.sin(solar_lon[0] * math.pi / 180), 3 *
                                                      math.sin(solar_lat[0] * math.pi / 180)), color=color.yellow,
                     shaftwidth=0.03, headwidth=.06, opacity=0)

    curve_list = []
    for i in range(1, len(mag_x) - 1):
        curve_list.append(curve(x=[spacecraft_x[i], spacecraft_x[i] + mag_x[i]],
                                y=[spacecraft_y[i], spacecraft_y[i] + mag_y[i]],
                                z=[spacecraft_z[i], spacecraft_z[i] + mag_z[i]], visible=False))
