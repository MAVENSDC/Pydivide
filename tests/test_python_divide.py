...
class TestPythonDivide(unittest.TestCase):
    ...
    def test_resample_Data(self):
        ...
        new_insitu = mvn_kp_resample(insitu, new_time)
        #Check some instruments to make sure the newly resampled data matches the old insitu when time is the same
        for obs in insitu['SPACECRAFT'].columns.values:
            self.assertEqual(insitu['SPACECRAFT'][obs]['2015-01-15T16:40:00'], new_insitu['SPACECRAFT'][obs]['2015-01-15T16:40:00'])
        for obs in insitu['APP'].columns.values:
            self.assertEqual(insitu['APP'][obs]['2015-01-15T16:40:00'], new_insitu['APP'][obs]['2015-01-15T16:40:00'])